...
function criarChat(pergunta = inputPergunta.value, resp = '...', data = '', emailUser, lengthPerguntas) {
    if(pergunta.length > 0 && qtnsPergunta < 4) {
        ...
        if(userConectado == 'true' || userConectado == true)  {
            try {
                //? Vai editar as perguntas e respostas feitas
                divPergunta.addEventListener('dblclick', () => {
        ...
        } else if(userConectado == true && resp != '...' || userConectado == 'true' && resp != '...') {
            respSpan.innerText = ''
                ...
                try {
                    for(let c = 0; c < User.Chat.length; c++) {
                        if(User.Chat[c].Id == idProdSelecionado) {
                            criarChat(User.Chat[c].Pergunta, User.Chat[c].Resposta, User.Chat[c].Data, User.Email, c)
                        }
